...
(function () {
  ...
  var BANDS = [
    { max: 1,        color: "#5fd75f", label: "Logged today" },
    { max: 3,        color: "#8fc74a", label: "1–3 days" },
    { max: 7,        color: "#fab219", label: "4–7 days" },
    { max: 30,       color: "#ec835a", label: "8–30 days" },
    { max: Infinity, color: "#ff7b7b", label: "30+ days / never" }
  ];
  function band(days) {
  ...
  function shell(inner, note) {
    return '' +
      '<div id="neglectCard" style="background:var(--card,#131f38);border:1px solid rgba(140,170,255,.14);border-radius:10px;' +
      'box-shadow:0 10px 28px rgba(2,7,18,.5);padding:16px 18px;margin-top:14px">' +
        '<div style="display:flex;align-items:baseline;justify-content:space-between;flex-wrap:wrap;gap:8px">' +
          '<div>' +
            '<div style="font-weight:700;color:var(--ink,#e9effd);font-size:15px">Maintenance Coverage</div>' +
            '<div style="font-size:12px;color:#9db0d5">Days since each machine last had a task logged</div>' +
          '</div>' +
          '<div style="font-size:11px;color:#64789f;text-align:right">' + (note || "") + '</div>' +
        '</div>' +
        inner +
      '</div>';
  }
  ...
  function legend() {
    return '<div style="display:flex;gap:14px;flex-wrap:wrap;margin:12px 0 4px;font-size:11px;color:#9db0d5">' +
  ...
  function locBlock(code, data) {
    ...
    var tiles = machines.map(function (m) {
      ...
      return '<div title="' + esc(m.n + " — " + m.s) + '" style="border:1px solid rgba(140,170,255,.14);border-left:5px solid ' + b.color +
        ...
        '</div>' +
        '<div style="font-size:10.5px;color:#64789f;margin-top:2px">Last: ' + esc(fmtDate(M.last)) + '</div>' +
        '</div>';
    }).join("");
    ...
    var head = '<div style="display:flex;align-items:baseline;justify-content:space-between;margin:16px 0 8px">' +
      '<div style="font-size:13px;font-weight:700;color:#e9effd">' + esc(loc.name) + ' <span style="font-weight:400;color:#64789f">· ' + esc(loc.full) + '</span></div>' +
      '<div style="font-size:11.5px;color:' + (flagged ? "#ff7b7b" : "#5fd75f") + '">' +
        (flagged ? flagged + ' of ' + machines.length + ' need attention' : 'all ' + machines.length + ' current') +
        ' · <span style="color:#9db0d5">' + L.count30 + ' logs / 30d</span>' +
      '</div></div>';
  ...
  function actionDot(st) {
    var c = st === "done" ? "#5fd75f" : (st === "open" ? "#64789f" : "#fab219");
  ...
  function recentActivityCard(rows) {
    ...
    var summary = Object.keys(LOCS).map(function (c) {
      ...
      var chips = ops.length ? ops.map(function (u) {
        return '<span style="display:inline-block;background:#0c1529;border:1px solid rgba(140,170,255,.14);border-radius:999px;' +
          'padding:2px 9px;font-size:11px;color:#9db0d5;margin:2px 4px 2px 0">' +
          esc(userShort(u)) + ' <span style="color:#64789f">· ' + p.users[u].n + '</span></span>';
      }).join("") : '<span style="font-size:11.5px;color:#64789f">no activity in the last 7 days</span>';
      return '<div style="margin:10px 0 2px">' +
        '<div style="font-size:12.5px;font-weight:700;color:#e9effd">' + esc(loc.name) +
          ' <span style="font-weight:400;color:#64789f">· ' + esc(loc.full) + '</span>' +
          '<span style="float:right;font-weight:600;color:' + (p.n ? "#5fd75f" : "#64789f") + '">' + p.n + ' entr' + (p.n === 1 ? 'y' : 'ies') + '</span></div>' +
    ...
    var feed = items.slice(0, MAXN).map(function (it) {
      ...
      return '<div style="display:flex;gap:8px;padding:6px 0;border-top:1px solid rgba(140,170,255,.1);font-size:12px;line-height:1.3">' +
        '<div style="flex:0 0 76px;color:#64789f;white-space:nowrap">' + esc(fmtWhen(it.ts)) + '</div>' +
        '<div style="flex:1 1 auto;color:#e9effd">' + actionDot(it.st) +
          '<span style="color:#9db0d5">' + esc(it.loc) + ' · ' + esc(it.machine) + '</span> — ' + esc(task) +
          ' <span style="color:#64789f">(' + esc(userShort(it.user)) + ')</span></div>' +
        '</div>';
    }).join("");
    var more = items.length > MAXN ? '<div style="font-size:11px;color:#64789f;margin-top:6px">+ ' + (items.length - MAXN) + ' more in the last 7 days</div>' : '';

    var inner =
      summary +
      '<div style="font-size:11px;font-weight:700;color:#64789f;letter-spacing:.04em;text-transform:uppercase;margin:14px 0 2px">Latest entries</div>' +
      (feed || '<div style="font-size:12px;color:#64789f;padding:8px 0">Nothing logged in the last 7 days.</div>') +
      more;

    return '' +
      '<div id="recentCard" style="background:var(--card,#131f38);border:1px solid rgba(140,170,255,.14);border-radius:10px;' +
      'box-shadow:0 10px 28px rgba(2,7,18,.5);padding:16px 18px;margin-top:14px">' +
        '<div style="display:flex;align-items:baseline;justify-content:space-between;flex-wrap:wrap;gap:8px">' +
          '<div>' +
            '<div style="font-weight:700;color:var(--ink,#e9effd);font-size:15px">Recent activity</div>' +
            '<div style="font-size:12px;color:#9db0d5">Everything logged in the last 7 days · all operators</div>' +
          '</div>' +
          '<div style="font-size:11px;color:#64789f;text-align:right">Updated ' + esc(fmtNow()) + '<br>' +
      ...
      Object.keys(LOCS).forEach(function (code) {
        ...
        for (var i = 0; i < nodes.length; i++) {
          ...
          if (t.indexOf(code + " — ") === 0 && n.children.length === 0) {
            ...
            s.style.cssText = "font-size:11px;color:#64789f;margin-top:6px";
  ...
  function refresh(force) {
    ...
    if (!sheetsReady()) {
      draw(shell('<div style="margin-top:12px;font-size:12.5px;color:#9db0d5">' +
    ...
    busy = true;
    draw(shell('<div style="margin-top:12px;font-size:12.5px;color:#9db0d5">Loading log data…</div>', ""));
    ...
    loadRows(force).then(function (rows) {
      ...
      draw(shell(legend() + body,
        ...
        ' · <a href="#" onclick="window.MM_COVERAGE.refresh(true);return false" style="color:#29a9ff;text-decoration:none">refresh</a>'));
      ...
      draw(shell('<div style="margin-top:12px;font-size:12.5px;color:#ff7b7b">Could not read the log sheet: ' +
  ...
  function banner(msg, ok) {
    ...
    el.style.background = ok ? "#0f2b18" : "#331014";
    el.style.color = ok ? "#5fd75f" : "#ff7b7b";
    el.style.border = "1px solid " + (ok ? "#0ca30c" : "#d03b3b");
    el.textContent = msg;
  ...
  function mmInjectStyle() {
    ...
    s.textContent = ".stats .stat.mm-active{outline:2px solid var(--blue,#29a9ff);outline-offset:1px;" +
      "box-shadow:0 2px 10px rgba(41,169,255,.25)}";
