...
(function () {
  ...
  function col(rate) {
    if (rate === null) return "var(--muted, #64789f)";
    return rate >= 85 ? "var(--green, #0ca30c)"
         : rate >= 50 ? "var(--amber, #fab219)"
         : "var(--red, #d03b3b)";
  }
      ...
      Object.keys(LOCS).forEach(function (l) {
        ...
        var minis = MINIS.map(function (m) {
          ...
          return '<div style="min-width:62px">' +
            ...
            '<div style="font-size:10.5px;color:var(--muted,#64789f)">' + m[1] +
              (p.a ? " " + p.d + "/" + p.a : "") + '</div>' +
            '</div>';
        }).join("");
        ...
        el.innerHTML =
          ...
          '<div class="val">' + fmtPct(dRate) +
            ' <span style="font-size:11px;font-weight:700;letter-spacing:1px;color:var(--muted,#64789f)">DAILY' +
            (daily.a ? " " + daily.d + "/" + daily.a : "") + '</span></div>' +
          '<div style="display:flex;gap:18px;flex-wrap:wrap;margin-top:7px;padding-top:7px;border-top:1px solid rgba(140,170,255,.14)">' +
            minis + '</div>' +
          '<div style="font-size:12px;color:var(--muted,#64789f);margin-top:6px">' +
